...
var is_sprinting = 0
enum Move_State  {IDLE, TURNING, WALKING, SPRINTING}
...
func _ready():
	...
	if Global.player_position_on_transition != Vector2() && Global.is_elsewhere: # Check if position is stored
...
func move(delta):
	...
	is_sprinting = 1 + 2 * int(Input.is_action_pressed("sprint"))
